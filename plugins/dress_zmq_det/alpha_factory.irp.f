...
! TODO
  ...
  do j=1,N_det_beta_unique
    ...
    do l_a=psi_bilinear_matrix_columns_loc(j), psi_bilinear_matrix_columns_loc(j+1)-1
      ...
      if (nt + exc_degree(i) <= 4) then
        ...
        indices(k) = psi_det_sorted_gen_from_det_sorted(indices(k))
        k=k+1
      endif
    enddo
  enddo
  ...
  if(psi_det_generators(1,2,i_generator) /= psi_det_sorted_gen(1,2,i_generator)) stop "gen <> sorted"
  do k=1,nmax
    i = indices(k)
...
subroutine alpha_callback_mask(delta_ij_loc, i_gen, sp, mask, bannedOrb, banned, indexes, indexes_end, rabuf, siz, iproc)
  ...
  !integer, allocatable :: rabuf(:)

  !allocate(rabuf(siz), abuf(siz), labuf(N_det), putten(N_det), det_minilist(N_int, 2, N_det))
  allocate(abuf(siz), labuf(N_det), putten(N_det), det_minilist(N_int, 2, N_det))
  
  !do i=1,siz
  !  rabuf(i) = psi_from_sorted_gen(rabufi(i))
  !end do

  do i=1,siz
    !psi_det_sorted_order
    abuf(i) = psi_det_sorted_from_det_sorted_gen(rabuf(i))

    if(psi_det_sorted_gen(1,1,rabuf(i)) /= psi_det_sorted(1,1,abuf(i))) stop "wrong conve11"
    !alpha(1,1) = psi_det_sorted(1,1,abuf(i))
    abuf(i) = psi_from_psi_sorted(abuf(i))
    !if(psi_det(1,1, abuf(i)) /= alpha(1,1)) stop "STAUPPe"
    if(psi_det_sorted_gen(1,1,rabuf(i)) /= psi_det(1,1,abuf(i))) stop "wrong conve"
  end do
...
subroutine count_pq(mask, sp, det, i_gen, N_sel, bannedOrb, banned, countedGlob, countedOrb, counted, interesting)
  ...
  integer                        :: i, s, j, k, l, h(0:2,2), p(0:4,2), nt
  ...
  do i=1, N_sel
    !ii = psi_from_sorted_gen(interesting(i))
...
subroutine splash_pq(mask, sp, det, i_gen, N_sel, bannedOrb, banned, indexes, abuf, interesting)
  ...
  integer                        :: i, j, k, l, h(0:2,2), p(0:4,2), nt, s
  integer(bit_kind)              :: perMask(N_int, 2), mobMask(N_int, 2), negMask(N_int, 2)
